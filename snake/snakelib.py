    ...
    def show(self):
        ...
        if self.game_over and self.want_board != "GameOver":
            ...
            for i in range(self.height - 1):
                ...
                self.printerr("")
            raise _IPyException("The game was over, but you did not did not indicate so. Please call ui.set_game_over() when the game is over.\n\n" + self.error_msg)
    ...
    def get_event(self):
        if self.events == []:
            if len(self.frames) == 0:
                ...
                raise _IPyException("You did not call ui.show() often enough: Either you forgot to call ui.show() before first get_event, or forgot to call show after an alarm : refresh event (including after ui.set_game_over()). \n\n" + self.error_msg)
        else:
            event = self.events[0]
...
class SnakeUserInterface(SnakeInterface):
    ...
    def board_size(self):
        return self.snake_interface.width, self.snake_interface.height
